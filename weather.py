...
meteo_responses = openmeteo.weather_api(meteo_url, params=meteo_params)
...
current = meteo_response.Current()
...
minutely_15 = meteo_response.Minutely15()
...
def calculate_dew_point(sensor_temp_c, sensor_humidity):
    print(f"Calculating dew point for Temperature: {sensor_temp_c}°C and Humidity: {sensor_humidity}%...")
    b = 17.62
    c = 243.12
    gamma = (b * sensor_temp_c / (c + sensor_temp_c)) + log(sensor_humidity / 100.0)
    sensor_dew_point = (c * gamma) / (b - gamma)
    print(f"Dew Point: {sensor_dew_point}°C")
    return sensor_dew_point

def calculate_feels_like(sensor_temp_c, sensor_humidity, wind_speed_mps):
